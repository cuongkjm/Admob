...
| `Platform/Ios/` | Objective-C++ delegates for Google Mobile Ads iOS SDK `13.5.0`, including `GADInterstitialAd`, `GADRewardedAd`, and banner delegates. |
...
| iOS | Real AdMob via Objective-C++ and Google Mobile Ads iOS SDK `13.5.0` APIs. |
...
The repository is intended to be consumed by `add_subdirectory(path/to/Admob)` or `FetchContent_MakeAvailable(QtAdMob)`. It avoids absolute local paths and parent-project global CMake settings.
